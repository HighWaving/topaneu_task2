...
RUN apt-get update && apt-get install -y --no-install-recommends git build-essential ca-certificates libgomp1 libglib2.0-0 && rm -rf /var/lib/apt/lists/*
...
ENV TASK2_DETECTOR_PYTHON=/opt/envs/detector/bin/python TASK2_REFINEMENT_PYTHON=/opt/envs/refinement/bin/python PYTHONDONTWRITEBYTECODE=1 GIT_PYTHON_REFRESH=quiet OMP_NUM_THREADS=2 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1
