...
class nnUNetPredictor(object):
    def __init__(self,
        ...
        self.model_weight_loaded = False
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            """
            print(f"[FOLD {i}] checkpoint trainer_name: {checkpoint['trainer_name']}")
            print(f"[FOLD {i}] checkpoint current_epoch: {checkpoint['current_epoch']}")
            print(f"[FOLD {i}] checkpoint inference_allowed_mirroring_axes: {checkpoint['inference_allowed_mirroring_axes']}")
            print(f"")
        ...
        # 20260609 HOUJING: Monkey patching getting trainer_name from env if exists
        trainer_name_from_env = os.environ.get('trainer_name', None)
        if trainer_name_from_env:
            print(f"[! ! ! ! ! !] Overwriting trainer_name from checkpoint ({trainer_name}) with trainer_name from env ({trainer_name_from_env})")
            trainer_name = trainer_name_from_env
        ...
        # HOUJING: This behavior from nnunet is problematic:
        #   nnunet Trainer is initiated during training
        #   Yet in inference, only the trainer class method is called
        # The inconsistency means that, if some of your network's settings are defined in the Trainer's __init__, it will not be configured during inference.
        # This leads to errors.
        # Here, we manually patch some configurations for inference, without creating the Trainer class.
        if trainer_name in (
            'Tr_rot30_Mirror01_TopK_1ceW_TopBrain_CT_MR_48fgCls_wModPresCls_cowPT_LRx0p1',
        ):
            from nnunetv2.training.nnUNetTrainer.variants.topbrain_generic_trainer.Tr_rot30_DiffClusterSM_TopK_ceW_wCls import UNetDecoder_wF_wCls
            from dynamic_network_architectures.architectures import unet
            UNetDecoder_wF_wCls.n_cls_tasks = 5
            UNetDecoder_wF_wCls.to_max_pool = False
            unet.UNetDecoder = UNetDecoder_wF_wCls
        elif trainer_name in (
            'Tr_rot30_Mirror01_TopK_FG_1ceW_TopBrain_CT_MR_48fgCls_wModPresCls_MaxPool_cowPT_LRx0p1',
        ):
            from nnunetv2.training.nnUNetTrainer.variants.topbrain_generic_trainer.Tr_rot30_DiffClusterSM_TopK_ceW_wCls import UNetDecoder_wF_wCls
            from dynamic_network_architectures.architectures import unet
            UNetDecoder_wF_wCls.n_cls_tasks = 5
            UNetDecoder_wF_wCls.to_max_pool = True
            unet.UNetDecoder = UNetDecoder_wF_wCls
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                prediction = self.predict_logits_from_preprocessed_data(data, reload_model_weight=reload_model_weight).cpu()
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        predicted_logits = self.predict_logits_from_preprocessed_data(dct['data'], reload_model_weight=reload_model_weight).cpu()
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor, reload_model_weight=True, to_cpu=True) -> torch.Tensor:
        """
        ...
        HOUJING:
            By default, the model weights are reloaded every time this function is called.
            However, if your predictor only has one set of weights (i.e., one fold), you can set
            `reload_model_weight` to False to avoid redundant loading and speed up inference.
            You can also set `to_cpu` to False to keep the prediction tensor on the GPU.
        """
        ...
        for params in self.list_of_parameters:
            ...
            if reload_model_weight or not self.model_weight_loaded:
                # messing with state dict names...
                if not isinstance(self.network, OptimizedModule):
                    self.network.load_state_dict(params)
                else:
                    self.network._orig_mod.load_state_dict(params)
                self.model_weight_loaded = True
