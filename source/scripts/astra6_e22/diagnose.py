import json
import numpy as np
from scripts.astra6_e22.prepare import RUN,P,DATA
...
for cid in ids:
 gt,aff,_=load_nifti(DATA/f'location_masks/{cid}.nii.gz');old,_,_=load_nifti(P/f'artifacts/astra6_e16_CT_expanded_segmentation_20260909/predictions_ct/{cid}.nii.gz');new,pa,_=load_nifti(RUN/f'predictions_ct/{cid}.nii.gz');assert np.allclose(aff,pa,atol=1e-4);cases[cid]={'E16':diagnose(gt,old,aff),'E22':diagnose(gt,new,aff)};print(cid,flush=True)
out={}
for v in ['E16','E22']:
...
write_json(RUN/'ADOPTION.json',{'adopt':pareto and out['E22']['matched']>=4 and out['E22']['location_correct']>=3,'official_pareto_gate':pareto,'official_delta':delta,'lesion_gate':out,'source_thresholds_unchanged':True})
