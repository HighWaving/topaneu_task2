import json
import numpy as np
from scripts.astra6_e17.prepare import RUN
...
for cid in ids:
 gt,aff,_=load_nifti(DATA/f'location_masks/{cid}.nii.gz');pr,pa,_=load_nifti(RUN/f'predictions/mr_center2_k05/{cid}.nii.gz');base,_,_=load_nifti(SHAPE/f'predictions/mr_center2_k05/{cid}.nii.gz');assert np.allclose(aff,pa,atol=1e-4);results[cid]=diagnose(gt,pr,aff);print(cid,flush=True)
...
write_json(RUN/'evaluation/lesion_diagnostics.json',{'E17':summary,'E04':old['versions']['E04'],'selected':len(candidate_rows),'kept':sum(r['filter_keep'] for r in candidate_rows),'cases':results});print(json.dumps(summary,indent=2),flush=True)
