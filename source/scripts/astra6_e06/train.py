...
def main():
 assert (RUN/'features/READY.json').exists();assert (RUN/'SOURCE_GEOMETRY_VALIDATED.json').exists();assert not (RUN/'model/LOCKED.json').exists();seed();records=[json.loads(s) for s in (RUN/'features/records.jsonl').read_text().splitlines()];tr=[i for i,r in enumerate(records) if not r['development']];dv=[i for i,r in enumerate(records) if r['development']];assert not {records[i]['group'] for i in tr}&{records[i]['group'] for i in dv}
 ...
 model=fit('final',list(range(len(records))),epochs=ep);write_json(RUN/'model/LOCKED.json',{'model_sha256':sha256_file(RUN/'model/final_last.pt'),'threshold_sha256':sha256_file(RUN/'model/THRESHOLD.json'),'code_sha256':sha256_file(Path(__file__)),'architecture_sha256':sha256_file(Path(__file__).with_name('common.py')),'config_sha256':sha256_file(RUN/'config.json'),'source':json.loads((RUN/'features/READY.json').read_text()),'frozen_E04_segmentation_sha256':sha256_file(P/'artifacts/astra6_e04_crop_segmentation_20260909T031146Z/model/final_last.pt'),'frozen_E02_candidate_assignment_sha256':sha256_file(P/'artifacts/astra6_e02_multiscale_vessel_signature_20260908T173916Z/candidate_predictions.jsonl'),'epochs':ep,'center2_used_for_fit_or_calibration':False});print('FULL_FORMAL_TRAINING_COMPLETE',flush=True)
