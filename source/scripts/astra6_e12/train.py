...
def main():
 ...
 assert not any('center2' in records[i]['case_id'] for i in all_ix)
 ...
 for cid in {records[i]['case_id'] for i in dv}:
  folder=P/('artifacts/fold1_source_center5_epoch60_20260909' if 'center5' in cid else 'artifacts/fold1_eval_center1_epoch60');bx,sc,_=load_boxes(folder/f'{cid}_boxes.pkl');pools[cid]={i for i,score,lo,hi in select_candidates(bx,sc)}
