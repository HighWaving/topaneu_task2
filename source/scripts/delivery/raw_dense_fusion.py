...
def main():
 ...
 p.add_argument('--bundle',type=Path,default=P/'submission_models_MRE32_CTE16_20260910_r3');p.add_argument('--classifier',type=Path)
 ...
 cfg=json.loads(a.calibration.read_text());q=cfg['arms']['fixed_control_plus_refined']['threshold'];d=cfg['control_D_threshold'];assert cfg['source45_only'];classifier=(a.classifier or a.bundle/'models/location_MR.joblib').resolve()
 ...
 detcode=a.bundle/'source/dependencies/detector_app';denv=env.copy();denv['PYTHONPATH']=os.pathsep.join([str(P),str(detcode.parent),str(detcode)]);denv['det_data']=str(V/'nndet_data');denv['det_models']=str(V/'nndet_models');checkpoint=a.bundle/'models/detector_MR'
 ...
 tenv=env.copy();tenv['PYTHONPATH']=os.pathsep.join([str(P/'vendor/delivery_runtime_deps'),str(app/'vendor')]);tenv['TOPANEU_MODEL_ROOT']=str(a.bundle/'models/ta36')
 ...
 stage('refinement',[a.refinement_python,'-m','scripts.delivery.refine_dense_fusion','--image',raw,'--predicted-vessel',vpath,'--boxes',a.work/'boxes/case_boxes.pkl','--dense',a.work/'dense/candidates.json','--classifier',classifier,'--segmentation',a.bundle/'models/segmentation_MR.pt','--detector-threshold',d,'--dense-threshold',q,'--policy',a.policy,'--output',a.output],P,env)
