...
R=P/'artifacts/astra6_e13_CT_anatomical_fp_filter_20260909';root=P/'artifacts/current_official_20260909';metrics=['PRECISION','RECALL','MCC','DICE','VOLSIM','HD95']
b=json.loads((root/'CT_E11/official.json').read_text());a=json.loads((root/'CT_E13/official.json').read_text());delta={k:a['overall'][k]-b['overall'][k] for k in metrics};br=json.loads((root/'CT_E11/per_case.json').read_text());ar=json.loads((root/'CT_E13/per_case.json').read_text());assert [r['case_id'] for r in br]==[r['case_id'] for r in ar];rng=np.random.default_rng(20260909);samples=[]
for _ in range(2000):
