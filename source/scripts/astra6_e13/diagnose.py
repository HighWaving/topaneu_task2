import json
import numpy as np
from scripts.astra6_e13.prepare import RUN,P,DATA
...
for cid in ids:
 gt,aff,_=load_nifti(DATA/f'location_masks/{cid}.nii.gz');old,_,_=load_nifti(P/f'artifacts/astra6_e11_CT_expanded_fp_filter_20260909/predictions_ct/{cid}.nii.gz');new,pa,_=load_nifti(RUN/f'predictions_ct/{cid}.nii.gz');assert np.allclose(aff,pa,atol=1e-4);cases[cid]={'E11':diagnose(gt,old,aff),'E13':diagnose(gt,new,aff)};print(cid,flush=True)
out={}
for v in ['E11','E13']:
