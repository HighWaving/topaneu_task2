...
def main():
 assert (RUN/'features/READY.json').exists();assert (RUN/'SOURCE_GEOMETRY_VALIDATED.json').exists();assert not (RUN/'model/LOCKED.json').exists();seed();records=[json.loads(s) for s in (RUN/'features/records.jsonl').read_text().splitlines()];all_ix=[i for i,r in enumerate(records) if '_ct_' in r['case_id']];tr=[i for i in all_ix if not records[i]['development']];dv=[i for i in all_ix if records[i]['development']];assert not {records[i]['group'] for i in tr}&{records[i]['group'] for i in dv}
 test=set(json.loads((RUN/'source_split.json').read_text())['fixed_CT5']);assert not {records[i]['case_id'] for i in all_ix}&test
 ...
 ep=fit('development',tr,dv);pred=json.loads((RUN/'model/development_best_predictions.json').read_text());pp=np.array(pred['p']);yy=np.array(pred['y'])
 from scripts.astra6_e01.e01_common import load_boxes,select_candidates
 pools={}
 for cid in {records[i]['case_id'] for i in dv}:
  bx,sc,_=load_boxes(P/f'artifacts/ct_fold2_all109_boxes/{cid}_boxes.pkl');pools[cid]={i for i,score,lo,hi in select_candidates(bx,sc)}
 deployed=np.array([records[i]['augmentation']==0 and records[i]['original_index'] in pools[records[i]['case_id']] for i in pred['rows']]);positive=deployed&(yy==1);assert positive.sum()>0
 threshold=float(np.nextafter(np.min(pp[positive]),0.));write_json(RUN/'model/THRESHOLD.json',{'threshold':threshold,'selection':'minimum source-dev original positive probability in actual score.3/top5 deployment pool, nextafter toward zero','source_deployed_positive_count':int(positive.sum()),'source_deployed_negative_count':int((deployed&(yy==0)).sum()),'source_recall':float(np.mean(pp[positive]>=threshold)),'source_negative_rejection':float(np.mean(pp[deployed&(yy==0)]<threshold)),'selected_epoch':ep,'CT5_used':False});write_json(RUN/'model/SELECTED_DURATION.json',{'epochs':ep,'max_epochs':100,'min_stop_epoch':30,'patience':20,'selection':'lowest balanced source-dev BCE'})

 model=fit('final',all_ix,epochs=ep);write_json(RUN/'model/LOCKED.json',{'model_sha256':sha256_file(RUN/'model/final_last.pt'),'threshold_sha256':sha256_file(RUN/'model/THRESHOLD.json'),'code_sha256':sha256_file(Path(__file__)),'architecture_sha256':sha256_file(Path(__file__).with_name('common.py')),'config_sha256':sha256_file(RUN/'config.json'),'source':{'shared_cache':json.loads((RUN/'features/READY.json').read_text()),'used_row_indices':all_ix,'used_case_ids':sorted({records[i]['case_id'] for i in all_ix}),'CT_rows_used':True,'MR_rows_used':False},'frozen_E04_segmentation_sha256':sha256_file(P/'artifacts/astra6_e04_crop_segmentation_20260909T031146Z/model/final_last.pt'),'frozen_CT_E09_classifier_sha256':sha256_file(P/'artifacts/astra6_e09_CT_expanded_location_20260909/model/classifier.joblib'),'epochs':ep,'CT2_other_cases_used':True,'CT5_used_for_fit_or_calibration':False});print('FULL_FORMAL_TRAINING_COMPLETE',flush=True)
