import json
import numpy as np
from scripts.astra6_e11.common import RUN,P,DATA
...
for cid in ids:
 gt,aff,_=load_nifti(DATA/f'location_masks/{cid}.nii.gz');old,_,_=load_nifti(P/f'artifacts/astra6_e09_CT_expanded_location_20260909/predictions_ct/{cid}.nii.gz');new,pa,_=load_nifti(RUN/f'predictions_ct/{cid}.nii.gz');assert np.allclose(aff,pa,atol=1e-4);cases[cid]={'E09':diagnose(gt,old,aff),'E11':diagnose(gt,new,aff)};print(cid,flush=True)
out={}
for v in ['E09','E11']:
