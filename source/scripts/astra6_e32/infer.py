 ...
 for n,cid in enumerate(ids,1):
  ...
  for old in [r for r in records if r['case_id']==cid]:
   ...
   if old['filter_keep']:
    ...
    lo,fg=native_foreground(pr,origin,step,image.shape);fallback=not fg.any();r.update(segmentation_arm='E32_'+arm,learned_S17_fallback=fallback,new_segmenter_voxels=int(fg.sum()),sampling_origin=origin.tolist(),sampling_step=step.tolist())
    if fallback:
