...
from scripts.astra6_e32.common import P,RUN
...
def main(arm):
 ...
 if not scope.exists():
  ...
  def audit(event,args):
   ...
   if not write and (path.is_relative_to(data) or path.is_relative_to(P/'artifacts')):
    ...
    reads.add(str(path))
  sys.addaudithook(audit);sys.argv=['predict_native','--image',str(image),'--predicted-vessel',str(vessel),'--boxes',str(boxes),'--classifier',str(classifier),'--segmentation',str(seg),'--fallback-segmentation',str(backup),'--fp-filter',str(fp),'--fp-threshold',str(threshold),'--arm',arm,'--device','cuda:0','--output',str(out)];runpy.run_module('scripts.astra6_e32.predict_native',run_name='__main__');active[0]=False;scope.write_text(json.dumps({'passed':True,'explicit_input_only':True,'reads':sorted(reads),'model_sha256':sha256_file(seg),'output_sha256':sha256_file(out),'TA36_and_D_provided_as_explicit_stage_inputs':True,'not_T4_or_whole_raw_pipeline_runtime':True},indent=2)+'\n')
 ...
 print('E32 explicit native entry',arm,'read-scope passed','parity',reference.exists(),flush=True)
