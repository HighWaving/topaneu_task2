...
R=P/'artifacts/astra6_e05_learned_location_splits_20260909';root=P/'artifacts/current_official_20260909'
b=json.loads((root/'E04/official.json').read_text());a=json.loads((root/'E05/official.json').read_text());metrics=['PRECISION','RECALL','MCC','DICE','VOLSIM','HD95'];delta={k:a['overall'][k]-b['overall'][k] for k in metrics}
br=json.loads((root/'E04/per_case.json').read_text());ar=json.loads((root/'E05/per_case.json').read_text());assert [x['case_id'] for x in br]==[x['case_id'] for x in ar];rng=np.random.default_rng(20260909);ds=[]
for _ in range(2000):
